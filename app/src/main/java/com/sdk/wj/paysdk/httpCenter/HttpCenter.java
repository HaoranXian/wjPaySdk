...
import com.sdk.wj.paysdk.utils.Kode;
...
public class HttpCenter {
	 ...
	 */
	public static void submitPostData(final String strUrlPath, final String content, final HttpListener listener) {
		new Thread(new Runnable() {
			@Override
			public void run() {
				if (Constants.isOutPut) {
					Log.debug("====>当前post请求内容:" + content);
				}
				...
				try {
					...
					if (response == HttpURLConnection.HTTP_OK) {
						...
						try {
							...
							if (Constants.isOutPut) {
								Log.debug("====>当前post请求返回内容：" + Kode.e(sb.toString()));
							}
	 ...
	 * get請求
	 */
	public static void submitGetData(final String urlString, final HttpListener listener) {
		new Thread(new Runnable() {
			@Override
			public void run() {
				...
				if (Constants.isOutPut) {
					Log.debug("====>当前get请求返回内容：" + resultData);
				}
			}
		}).start();
	}

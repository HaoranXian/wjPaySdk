...
public class JsonUtil {
	...
	public static String buildJson(Object obj) {
		...
		try {
			...
			Log.debug("构建json字符串时出错：" + e.getMessage());
		}
		return null;
	}
	...
	public static JsonEntity.JsonInterface[] parseJSonArray(Class<?> clz, String jsonString) {
		...
		try {
			// jsonString =
			// "{\"b\":{\"f\":\"com.zhangdan.app\",\"g\":\"帮您轻松管理信用卡:51信用卡管家\",\"d\":\"51信用卡管家\",\"e\":2,\"b\":\"adbdddd\"},\"c\":{\"b\":\"description success\",\"a\":0}}";
			JSONObject jo = new JSONObject(jsonString);
			...
			if (!jo.isNull(ji.getShortName())) {
				...
				if (ja != null) {
					...
					for (int i = 0; i < ja.length(); i++) {
						ji = (JsonEntity.JsonInterface) clz.newInstance();
	...
	public static String readMonthJSON(String content){
		try {
